...
```bash
./scripts/setup.sh  # 自动安装 uv 并同步依赖
```
...
本项目使用 [uv](https://github.com/astral-sh/uv) 进行依赖管理，相比传统的 pip + venv 有以下优势：

- ⚡️ **更快的安装速度**：比 pip 快 10-100 倍
- 🔒 **可靠的依赖锁定**：通过 `uv.lock` 确保环境一致性
- 🎯 **单一配置源**：只需维护 `pyproject.toml`
- 🚀 **简化的工作流**：`uv run` 自动管理虚拟环境
