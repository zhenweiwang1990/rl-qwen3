...
def search_emails(
    ...
    """Search emails in the database. Returns list of SearchResult objects with message_id and snippet.
...
def read_email(message_id: str) -> Optional[Email]:
    """Read a single email by message_id. Returns Email object with all details, or None if not found.
...
def return_final_answer(answer: str, sources: List[str] | None) -> str:
    """Return the final answer to the user's query. This function should be called when the agent has found the answer. If the answer cannot be found, return "I don't know" with empty sources.
